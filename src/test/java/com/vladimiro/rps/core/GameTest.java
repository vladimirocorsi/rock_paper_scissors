...
public class GameTest {
	...
	public void test_paper_vs_rock() {
		...
		doNothing().when(player1).notifyOpponentSymbol(Mockito.any());
		when(player2.askForSymbol()).thenReturn(Symbol.ROCK);
		doNothing().when(player2).notifyOpponentSymbol(Mockito.any());
		...
		verify(player1).notifyOpponentSymbol(Mockito.any());
		verify(player2).notifyOpponentSymbol(Mockito.any());
		...
		assertEquals(0, game.getScore2());
		verify(player1, times(2)).notifyOpponentSymbol(Mockito.any());
		verify(player2, times(2)).notifyOpponentSymbol(Mockito.any());
	}
